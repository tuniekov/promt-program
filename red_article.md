...
Но при тестировании, выяснилось, что нейросеть Cline не допоняла и вместо направления http запросов в БЯМ пишет для них обычный сайт.
...
# Выводы
Идея промпт-программы выглядит интересно. Даже возможно может реализовать. Но куча денег и не понимание нейросетью задач, для которых она не тренирована делает идею сомнительной. То есть, пока буду действовать как все: включать запросы к нейросети код классической программы, когда это выгодно.
